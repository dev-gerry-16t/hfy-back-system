...
const executeSetUserProfile = async (params, res, url, file) => {
  ...
  const fileName =
    isEmpty(documentName) === false
      ? documentName.split(".")
      : ["none", "none"];
  const fileType = fileName[fileName.length - 1];
  const storeProcedure = "authSch.USPsetUserProfile";
  try {
